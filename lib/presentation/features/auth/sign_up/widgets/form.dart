...
class SignUpForm extends StatelessWidget {
  ...
  Widget _buildFirstNameField(BuildContext context) {
    return BlocBuilder<SignUpBloc, SignUpState>(
      buildWhen: (previous, current) =>
          previous.firstName != current.firstName || previous.hadBeenSubmitted != current.hadBeenSubmitted,
      builder: (context, state) {
        return AppTextField(
          autofocus: true,
          hintText: context.l10n.field__first_name,
          errorText: state.hadBeenSubmitted && state.firstName.isEmpty ? context.l10n.field__first_name_required : null,
  ...
  Widget _buildSurNameField(BuildContext context) {
    return BlocBuilder<SignUpBloc, SignUpState>(
      buildWhen: (previous, current) =>
          previous.surName != current.surName || previous.hadBeenSubmitted != current.hadBeenSubmitted,
      builder: (context, state) {
        return AppTextField(
          hintText: context.l10n.field__surname,
          textInputAction: TextInputAction.next,
          textCapitalization: TextCapitalization.words,
          errorText: state.hadBeenSubmitted && state.surName.isEmpty ? context.l10n.field__surname_required : null,
          onChanged: (value) => context.read<SignUpBloc>().add(SurNameChanged(value)),
        );
      },
    );
  }
